/** Throwaway seed for browser verification. */
import { closeDb, getDb } from "@/db";
import * as S from "@/db/schema";
import { hashPassword } from "@/lib/auth";
import { createPackage, createProject, addFormLine, listFormLines } from "@/lib/projects";
import { importSubs, parseSubImport } from "@/lib/subs";
import { sendInvites } from "@/lib/invites";
import { requirePortal, submitPortalBid } from "@/lib/portal";
import { uploadPlanFile } from "@/lib/plan-files";
import { addAdjustment } from "@/lib/leveling-data";
...
async function main() {
  ...
  const [company] = await db.insert(S.companies).values({
    name: "Fulton Build Group", plan: "builder",
    replyToEmail: "estimating@fultonbuild.example",
    trialEndsAt: new Date(Date.now() + 11 * 86400000),
    settings: S.DEFAULT_SETTINGS,
  }).returning();
  const [user] = await db.insert(S.users).values({
    companyId: company.id, email: "kyle@fultonbuild.example", name: "Kyle Ferrand",
    passwordHash: await hashPassword("fulton-yard-2026"), role: "admin",
  }).returning();
  ...
  const project = await createProject(company.id, "builder", actor, {
    ...
    bidDueAt: new Date(Date.now() + 6 * 86400000),
    notes: "Owner meeting the 24th. Tenant wants mechanical broken out.",
  });
  const pkg = await createPackage(company.id, project.id, actor, {
    csiDivision: "26",
    scopeNotes: "Base bid excludes owner-furnished fixtures. Include all permits and inspections. Night work weeks 3-4.",
    seedForm: true,
  });
  await createPackage(company.id, project.id, actor, { csiDivision: "23", scopeNotes: "Two RTUs, curbs by us.", seedForm: true });
  await createPackage(company.id, project.id, actor, { csiDivision: "09", scopeNotes: "Level 4 finish throughout.", seedForm: true });
  await addFormLine(company.id, pkg.id, {
    description: "ALT 1: Site lighting poles", unit: "EA", quantity: "6", isAlternate: true, isAllowance: false,
  });
  await uploadPlanFile(company.id, "builder", actor, {
    projectId: project.id, packageId: null, filename: "Fulton-B-E-sheets.pdf",
    contentType: "application/pdf", data: Buffer.from("%PDF-1.4 " + "x".repeat(4000)),
    versionLabel: "Permit set, Rev 2", supersedes: null,
  });

  const contacts = await db.select({ c: S.subContacts, s: S.subCompanies })
    .from(S.subContacts).innerJoin(S.subCompanies, (await import("drizzle-orm")).eq(S.subContacts.subCompanyId, S.subCompanies.id));
  const elec = contacts.filter((r) => r.s.trades.includes("26") && r.s.name !== "Pike Street Electric");
  ...
  // Pike Street gets invited but stays silent, so the board has a real mix.
  const pike = contacts.find((r) => r.s.name === "Pike Street Electric")!;
  const pikeInvite = await sendInvites(company.id, actor, { packageId: pkg.id, subContactIds: [pike.c.id], personalNote: null });
  ...
  const { loadLevelingPage } = await import("@/lib/leveling-data");
  ...
  await addAdjustment(company.id, actor, {
    packageId: pkg.id, bidId: bCol.bid.id, bidFormLineId: at("Fire alarm"),
    kind: "plug", amountCents: 1_230_000, reason: "Lump sum excludes fire alarm",
  });
  await addAdjustment(company.id, actor, {
    packageId: pkg.id, bidId: bCol.bid.id, bidFormLineId: null,
    kind: "scope_add", amountCents: 240_000, reason: "Dumpsters not carried in the lump sum",
  });
  await addAdjustment(company.id, actor, {
    packageId: pkg.id, bidId: mCol.bid.id, bidFormLineId: at("Fire alarm"),
  ...
  await askPortalQuestion(h, "Is the fire alarm rough-in in this package, or is 28 buying it out separately?");

  console.log(JSON.stringify({
    login: { email: user.email, password: "fulton-yard-2026" },
    projectId: project.id,
    pkgId: pkg.id,
    portal: invited.links.find((l) => l.subCompany === "Brightline Electric")!.url,
