 ...
 *
 * Created on first use, so importing this during `next build` (no DATABASE_URL)
 * neither opens a socket nor throws.
 *
 ...
 * is one connection with a short idle timeout; a long-lived process gets a real
...
/** Close the pool — used by the standalone worker and by scripts. */
