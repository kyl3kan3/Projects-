 ...
 * `next build` — which has no DATABASE_URL — never opens a socket or throws.
 *
 * Serverless note: on Vercel each warm function instance keeps its own pool, so
 * a generous `max` multiplies into Neon's connection ceiling fast. There the
 * pool is one connection with a short idle timeout; a long-lived worker gets a
 * real pool. `prepare: false` is required in both: Neon's pooled endpoint runs
