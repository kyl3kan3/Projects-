...
3. **DESIGN.md** — a redline spec, not inspiration: exact palette hexes, type
   roles with sizes, spacing steps, component construction, the sign & lock
...
- **A note is not a note until a human signs it.** There must be no code path
  from draft to signed record without an explicit clinician signature, and
  signed versions are immutable — amendments create new signed versions.
  These two invariants get tests before they get UI.
- **PHI discipline everywhere:** job payloads carry IDs only (no PHI in
  Redis); no PHI in emails, logs, error reports, or analytics; audio and
  transcripts honor `purge_at`; every access writes an audit event.
...
- **No gradient fills or glows on interactive elements.** Primary button is
  ink fill with paper text on this light ground.
...
- **Real content everywhere** — no lorem, no gray placeholder bars; plausible
  clinical data (clearly fictional client labels) in every screen and empty
  state.
...
2. Design tokens + global CSS from DESIGN.md **before any screen** — type
   roles, spacing, buttons, rows, the note sheet, the sign & lock keyframes.
...
4. The pipeline end to end (capture -> transcribe -> draft) with worker jobs,
   then the review room and the sign gate, per ARCHITECTURE.md key flows 1-2.
5. Retention purge, audit surfaces, exports, and billing (flows 3-5).
6. Product screens in ROADMAP week order, mobile-first, wiring real flows
   end to end (no mocked handlers left behind).
7. Landing page **last**, to MARKETING_PLAYBOOK.md: name the enemy (pajama
   time), show the draft assembling against the between-sessions clock within
   5 seconds, honest receipts (never fabricated), one CTA phrase repeated
   verbatim: **"Start free — 14 days"**.
...
- The two signing invariants (no unsigned filing path; signed immutability)
  are covered by tests that fail if violated.
