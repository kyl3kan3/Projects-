/**
 * Control-plane database client (lazy singleton).
 *
 * The connection opens on first use, so importing this during `next build`
 * (no DATABASE_URL) neither opens a socket nor throws.
 *
 * Serverless note: on Vercel every warm instance keeps its own pool, so a
 * generous `max` multiplies into Neon's connection ceiling fast. There the pool
