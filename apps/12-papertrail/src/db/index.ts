 ...
 * pool is one connection with a short idle timeout. `prepare: false` is required
 * either way: Neon's pooled endpoint is PgBouncer in transaction mode, which
 * cannot carry prepared statements across connections.
 */
...
/** Close the pool — used by scripts and the test harness on shutdown. */
