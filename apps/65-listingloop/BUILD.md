...
- **No gradient fills or glows on interactive elements.** Primary button is
  ink fill with manila text.
- **One accent, rationed to ≤10% of any screen** — keybox maroon marks
  today, at-risk, and focus only.
- **The date engine is the product** — pure, exhaustively tested
  (business days, weekend rolls, holiday observance, month boundaries),
  and every computed date can explain itself in a sentence. Anchor
  edits ALWAYS preview a diff before applying; sent reminder history is
  never rewritten.
- **Reminders are exactly-once** per (date, offset) via the ledger, with
  per-party digest coalescing.
- **Desktop-first console (1280px); the party portal is mobile-first at
  390px** — clients read it from phones.
- **Fonts must actually load** (self-hosted woff2, preloaded).
- **Real content everywhere** — plausible deals, dates, and parties; no
  lorem.
...
2. Design tokens + global CSS from DESIGN.md **before any screen** —
   type roles, the deal line and its nodes, the unfurl keyframes.
3. Data model (`src/db/schema.ts` is complete — migrate it), auth,
   holiday data load.
4. The date engine first, pure, with its test suite — then deals +
   instantiation + the timeline render with derivation sentences.
5. Anchor-edit diff preview/apply, the reminder ledger + fan-out,
   then documents/portal/commissions — in ROADMAP milestone-1 order.
6. Landing page **last**, to MARKETING_PLAYBOOK.md: name the enemy
   (the critical date that lived only in the contract PDF), show the
   product running within 5 seconds, this app's one device ("Every
   deadline on the contract, on one line."), honest receipts (demo
   data labeled), one CTA phrase repeated verbatim: **"Start free —
   14 days"**.
