 ...
 * Dependency-free on purpose; it walks `src/` and turns DESIGN.md and
 * DESIGN_LANGUAGE.md into assertions:
 *
 ...
 *     `postgres`, `net` and `tls` end up in a browser bundle. Caught a real one
 *     in this app (Placard importing lib/deals for its status labels).
 *  6. Every exported "use server" function is called from somewhere. An unused
 *     one is a public endpoint nothing needs.
 ...
 *     the build catches.
 *  8. No `new Date("yyyy-mm-dd")` on a bare ISO date: that is midnight UTC, and
 *     this product's entire domain is calendar dates. `toDayNumber` exists.
 *  9. Only the three radii DESIGN.md names.
 */
...
for (const file of files) {
  ...
  for (const m of source.matchAll(/(?:^|\n)\s*import\s+([^;]*?)from\s+["'](@\/[^"']+)["']/g)) {
    // `import type { X } from` is erased by the compiler and cannot pull a
    // runtime module into a bundle, so it is not an edge.
...
const RADII = new Set(["2px", "6px", "8px", "50%", "0"]);
...
for (const file of files) {
  ...
  // The timeline PDF has to name its colours in rgb() for pdf-lib; its header
  // says so and the values are the tokens.
  const isPdf = /src\/lib\/packet\.ts$/.test(file);
  // lib/theme.ts holds the single literal platform metadata needs; its header
  ...
  lines.forEach((line, i) => {
    ...
    for (const hex of line.match(HEX) ?? []) {
      ...
      if (!isTokens && !isTest && !isPdf && !isThemeToken) {
    ...
    if (/new Date\(\s*["'`]\d{4}-\d{2}-\d{2}["'`]\s*\)/.test(line) && !isTest) {
      fail(
        file,
        n,
        'new Date("yyyy-mm-dd") is midnight UTC and moves the date west of Greenwich — use toDayNumber/addDays',
      );
    }
