 ...
 * use, so importing this during `next build` (no DATABASE_URL) neither opens a
 * socket nor throws.
 *
 ...
 * is one connection with a short idle timeout; the long-lived worker gets a real
 * one. `prepare: false` is required either way — Neon's pooled endpoint runs
