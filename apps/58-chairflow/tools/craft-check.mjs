 ...
 *
 * The checks a type-checker cannot make and `next build` will not fail on. Dependency-free on
 * purpose; it walks `src/` and turns DESIGN.md, DESIGN_LANGUAGE.md and a list of defects that
 * shipped inside green builds elsewhere in this portfolio into assertions.
 *
 *   1. No colour literal outside `globals.css`. Tokens exist so a screen cannot invent a hue;
 *      this is how that stops being a promise. `lib/theme.ts` is exempt and says why.
 ...
 *   5. Only the three radii DESIGN.md names.
 *   6. No client component may reach the database client — that is exactly how `postgres`,
 *      `net` and `tls` end up in a browser bundle.
 *   7. Every exported `"use server"` function is called somewhere. An unused one is a public
 *      endpoint nothing needs.
 *   8. A `"use server"` module exports **only functions**. A constant exported beside the
 *      actions compiles, then arrives `undefined` on the client and crashes the screen on first
 *      interaction.
 *   9. No `Date` interpolated into a raw `sql` fragment: it bypasses Drizzle's column encoder
 *      and throws inside postgres.js at runtime, which nothing in the build catches.
 *  10. Money columns are `bigint`, never `integer`: a currency total in int4 stops at $21.4M.
 *  11. No `<form>` inside another `<form>`, and none inside a `<Link>`/`<a>`. The browser drops
 *      the inner one and its submit runs the outer action.
 *  12. No `startTransition` — this codebase drives every deferred submit through
 *      `form.requestSubmit()`, because `startTransition` inside a `setState` updater throws in
 *      React 19 and surfaces as "a client-side exception has occurred".
 *  13. No `<Link>` to `/api/…`: `next/link` prefetches on hover, so a GET with a side effect
 *      runs before anybody taps it.
 *  14. `.gitignore` must not ignore `drizzle/meta/` — it holds `_journal.json`, without which a
 *      fresh clone cannot run `db:migrate`.
 */
...
for (const file of files) {
  ...
  for (const m of source.matchAll(/(?:^|\n)\s*import\s+([^;]*?)from\s+["'](@\/[^"']+)["']/g)) {
    // `import type { X } from` is erased by the compiler and cannot pull a runtime module
    // into a bundle, so it is not an edge.
...
function touchesDb(file, seen = new Set()) {
  ...
  seen.add(file);
  // A "use server" module imported by a client component is replaced with a reference at
  // build time, so it is not an edge into the browser bundle.
  const source = read(file);
...
const RADII = new Set(["8px", "12px", "20px", "50%", "4px", "0"]);
...
for (const file of files) {
  ...
  // lib/theme.ts holds the single literal platform metadata needs; its header explains why it
  // cannot be a CSS custom property.
  const isThemeToken = /src\/lib\/theme\.ts$/.test(file);
  // The brand mark is an SVG file rendered by the browser before any stylesheet exists.
  const isIconAsset = /icon\.svg$/.test(file);

  // Comments describe hazards; they do not commit them. Blank the comment lines once, and
  // every pass below reads the same code-only view.
  const code = [];
  ...
  code.forEach((line, i) => {
    ...
    for (const hex of line.match(HEX) ?? []) {
      ...
      if (!isTokens && !isTest && !isThemeToken && !isIconAsset) {
      ...
      for (const m of line.matchAll(/<\/?(form|Link|a)\b([^>]*)>?/g)) {
        ...
        if (tag === "form") {
          if (closing) depth = Math.max(0, depth - 1);
            ...
            if (depth > 0) fail(file, n, "a <form> nested inside another <form> — the browser drops the inner one");
            if (linkDepth > 0) fail(file, n, "a <form> inside a <Link>/<a> — the tap navigates instead of submitting");
