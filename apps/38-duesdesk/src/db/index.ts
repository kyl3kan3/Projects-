 ...
 *
 * The connection is opened on first use, so importing this module during
 * `next build` — which has no DATABASE_URL — neither opens a socket nor throws.
 *
 * On Vercel every warm function instance keeps its own pool, so `max` is held
 * at one connection there and the idle timeout kept short; a long-lived local
...
/** Close the pool. Used by scripts, never by a request path. */
