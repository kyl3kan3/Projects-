...
- **One accent, rationed to ≤10% of any screen.** Semantic colors carry
  meaning only.
- **Mobile-first at 390px.** The driver flow is a PWA used with gloves in
  sun glare — 44px targets and AAA contrast are load-bearing, not polish.
- **Offline is a feature, not an error state.** Inspection drafts persist
  locally and sync exactly once; a silent drop is a failed build.
...
2. Design tokens + global CSS from DESIGN.md **before any screen** — type
   roles, spacing, buttons, tiles, item rows, the stamp keyframes.
3. Data model (`src/db/schema.ts` is complete — migrate it), auth, driver
   tokens, and API/services per ARCHITECTURE.md.
4. The driver flow next, end to end on a real phone: token link -> item
   groups -> photo-on-fail -> odometer -> signature -> submit, including
   the offline draft path. This surface is the product.
5. Then the office loop: defect processing, tickets, service history,
   reminders, exports — screens in ROADMAP milestone-1 order, mobile-first,
...
6. Landing page **last**, to MARKETING_PLAYBOOK.md: name the enemy (the
   clipboard), show the product running within 5 seconds, this app's one
   device ("The pre-trip that takes 90 seconds, not a clipboard."), honest
...
- The airplane-mode test passes: an inspection drafted offline syncs
