...
- **One accent, rationed to ≤10% of any screen.** Semantic colors carry
  meaning only.
- **Mobile-first at 390px.** The cleaner's job page is the most phone-bound
  surface in the portfolio — it must be flawless one-handed.
...
2. Design tokens + global CSS from DESIGN.md **before any screen** — type
   roles, spacing, buttons, tiles, rows, the tile-flip keyframes.
3. Data model (`src/db/schema.ts` is complete — migrate it), auth, job
   tokens, and API/services per ARCHITECTURE.md.
4. The queue spine next: iCal sync -> stay diff -> turnover re-flow ->
   cleaner notification, end to end against a real Airbnb iCal URL.
5. Product screens in ROADMAP milestone-1 order, mobile-first, wiring real
   flows end to end (no mocked handlers left behind). The cleaner job page
   and its photo gate come before the host dashboard polish.
6. Landing page **last**, to MARKETING_PLAYBOOK.md: name the enemy (taking
   "it's done" on faith), show the product running within 5 seconds, this
   app's one device ("The turnover photographed clean before the next guest
   lands."), honest receipts (never fabricated), one CTA phrase repeated
