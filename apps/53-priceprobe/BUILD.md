...
- **One accent, rationed to ≤10% of any screen.** Semantic colors carry
  meaning only — and in this product they mark *your exposure*, never
  market direction.
- **Suggestions never auto-push.** There must be no code path that writes
  a price to any store. Read-only is architectural, not a setting.
- **Scrape politely.** Per-domain rate limits with jitter, robots
  awareness, hash short-circuits, honest failure states. Hammering a
  domain is a build failure.
...
2. Design tokens + global CSS from DESIGN.md **before any screen** — type
   roles, spacing, buttons, rows, ladders, the overnight-reveal keyframes.
3. Data model (`src/db/schema.ts` is complete — migrate it), auth, and
   API/services per ARCHITECTURE.md.
4. The scrape spine next, end to end against the Phase-0 fixture pages:
   track-a-page preview -> scheduler -> polite fetch -> snapshot ->
   change event -> Slack alert. The per-domain limiter and failure
   ladder are part of this milestone, not polish.
5. Product screens in ROADMAP milestone-1 order, mobile-first, wiring
   real flows end to end (no mocked handlers left behind): positions,
   SKU detail with ladder + history, suggestions, digest.
6. Landing page **last**, to MARKETING_PLAYBOOK.md: name the enemy (the
   silent undercut), show the product running within 5 seconds, this
   app's one device ("Your price position while you slept."), honest
...
- The politeness tests pass: per-domain spacing, domain backoff, hash
  short-circuit, sanity quarantine.
